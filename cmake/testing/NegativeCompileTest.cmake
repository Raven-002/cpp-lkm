# NegativeCompileTest.cmake
# Provides add_negative_compile_test and add_negative_link_test macros.
#
# Reads flags from the my_kernel_module.iface INTERFACE target at configure
# time so no global LIBKERNEL_* variables are needed.

# Collect flags from the interface target into string variables usable in
# add_custom_target commands. This runs at configure time; genexes that
# need build-time evaluation are handled via file(GENERATE) where needed.
...
macro(add_negative_compile_test TARGET_NAME SOURCE_FILE)
    ...
    add_custom_target(
        ${TARGET_NAME}
        COMMAND
            ${CMAKE_COMMAND} -DCOMPILER=${CMAKE_CXX_COMPILER}
            "-DCXX_FLAGS=${_neg_cxx_flags}" -DSOURCE=${SOURCE_FILE} -P
...
macro(add_negative_link_test TARGET_NAME SOURCE_FILE)
    ...
    add_custom_target(
        ...
        COMMAND
            ${CMAKE_COMMAND} -DCOMPILER=${CMAKE_CXX_COMPILER}
            "-DCXX_FLAGS=${_neg_cxx_flags}" "-DLINK_FLAGS=-Wl,--no-undefined"
            -DSOURCE=${SOURCE_FILE} "-DLIB_TO_LINK=$<TARGET_FILE:my_kernel_module>" -P
